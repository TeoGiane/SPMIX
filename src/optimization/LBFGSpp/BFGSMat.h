...
public:
    ...
    inline void compute_H(int dim, Matrix & res)
    {
        // std::cout << std::endl;
        // std::cout << "compute_H()" << std::endl;
        // std::cout << "dim: " << dim << std::endl;
        res.resize(dim, dim);
        Vector v, res_col;
        for (size_t i = 0; i < dim; i++)
        {
            v = Vector::Zero(dim); v[i] = 1;
            apply_Hv(v, Scalar(1), res_col);
            res.col(i) = res_col;

            // std::cout << "i: " << i << std::endl;
            // std::cout << "v: " << v.transpose() << std::endl;
            // std::cout << "res_col: " << res_col.transpose() << std::endl;
        }
        // std::cout << std::endl;
    }
